...
String.formatMax = function() {
	if (arguments.length == 0)
		return null;
	var s = arguments[0];
	var reg = new RegExp('32', "gm");
...
(function($) {
	...
	$.fn.pannel = function(options) {
		...
		for (var i = 0, max = items.length; i < max; i++) {
			itemId = items[i].url.split('/').join('.');
			sb.append(String.format('<li><a id="{0}">{1}</a></li>', itemId, items[i].title));
		}
	    ...
	    $lis.bind('click', function() {
			...
			_clearInterval();
			resetTime();
            window.location.href = href + "#" + opts.id + "#" + index;
